...
router.post("/insert", async function (req, res) {
    ...
    if (formData.fields.name_english && formData.fields.name_aramaic) {
        ...
        let ifExist = await City.countDocuments({ name_english: formData.fields.name_english, name_aramaic: formData.fields.name_aramaic, is_show: 1 })
        if (ifExist) {
        ...
        newcity.save(async function (saveError, saveSuccess) {
            ...
            if (saveSuccess) {
                var response = { status: 1, message: 'City addedd successfully', data: saveSuccess }
                res.send(response);
            }
            else {
...
router.post("/get", async function (req, res) {
    ...
    City.countDocuments(searchQuery, function (err, totalCount) {
        if (err) {
            response = { "status": false, "message": "Error fetching data" }
        }
        City.find(searchQuery, {}, query, async function (err, data) {
...
router.post("/delete", async function (req, res) {
    let formData = await commonHelper.getFormRecords(req)
    City.findOneAndUpdate({ _id: formData.fields.id }, { is_show: 0 }, function (err, slide) {
        if (err) throw err
        var response = { status: 1, message: 'City deleted successfully', data: slide }
        res.send(response);
    });
});
...
router.post("/active", async function (req, res) {
    let formData = await commonHelper.getFormRecords(req)
    City.findOneAndUpdate({ _id: formData.fields.id }, { status: 1 }, function (err, slide) {
        if (err) throw err
        var response = { status: 1, message: 'City Activated successfully', data: slide }
        res.send(response);
    })
});
...
router.post("/inactive", async function (req, res) {
    let formData = await commonHelper.getFormRecords(req)
    City.findOneAndUpdate({ _id: formData.fields.id }, { status: 0 }, function (err, slide) {
        if (err) throw err
        var response = { status: 1, message: 'City De-activated successfully', data: slide }
        res.send(response);
    });
});
...
router.post("/getSingle", async function (req, res) {
    let formData = await commonHelper.getFormRecords(req)
    City.findOne({ _id: formData.fields.id }, function (err, slide) {
        if (err) throw err
        console.log(slide)
        if (slide) {
...
router.post("/update", async function (req, res) {
    ...
    if (formData.fields.id) {
        if (formData.fields.name_english && formData.fields.name_aramaic) {
            ...
            let ifExist = await City.countDocuments({ name_english: formData.fields.name_english, name_aramaic: formData.fields.name_aramaic, is_show: 1, _id: { $ne: formData.fields.id } })
            if (ifExist) {
                var response = { status: 0, message: 'Name Already Exist', dataflow: 0, data: {} }
                res.send(response);
                return
            ...
            City.findOneAndUpdate({ _id: formData.fields.id }, {
                name_english: formData.fields.name_english,
                name_aramaic: formData.fields.name_aramaic,
            }, function (err, slide) {
                if (err) throw err
