...
export class CityComponent implements OnInit {
  ...
  getUsers = async (pageNo, searchItem, dateRange?) => {
    ...
    this.cmsservice.getcity(UsersList).subscribe((res) => {
      this.data = res;
  ...
  InactiveUser = async (Id, indexIs) => {
    ...
    if (confirm('Are you sure you want to Change status?')) {
      ...
      await this.cmsservice.inactivecity(newformData).subscribe((response) => {
  ...
  ActiveUser = async (Id, indexIs) => {
    if (confirm('Are you sure you want to Change status?')) {
      ...
      const newformData = new FormData();
      newformData.append('id', Id);
      await this.cmsservice.activecity(newformData).subscribe((response) => {
        if (response['status'] == 1) {
          this.toastrService.show('success', response['message'], { status: 'success' });
          this.ngOnInit()
        } else {
  ...
  deleteUser = async (Id, Indexis) => {
    if (confirm('Are you sure you want to Delete?')) {
      ...
      await this.cmsservice.deletecity(newformData).subscribe((response) => {
        if (response['status'] == 1) {
