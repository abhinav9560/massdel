...
@Component({
  selector: 'ngx-trainingadd',
  templateUrl: './trainingadd.component.html',
  styleUrls: ['./trainingadd.component.scss']
})
export class TrainingaddComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.id) {
      ...
      this.cmsservice.getSingletraining(newformData).subscribe(res => {
  ...
  userSubmit = async (formData) => {
    ...
    if (!this.id) {
        ...
        this.cmsservice.inserttraining(newformData).subscribe((response) => {
          this.common.hideLoader()
        ...
        newformData.append('name_english', formData.name_english.trim());
        newformData.append('name_aramaic', formData.name_aramaic.trim());
        this.cmsservice.updatetraining(newformData).subscribe((response) => {
          this.common.hideLoader()
          if (response['status'] == 1) {
           this.ngOnInit()
            this.toastrService.show('success', response['message'], { status: 'success' });
