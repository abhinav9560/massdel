...
@Component({
  selector: 'ngx-languageadd',
  templateUrl: './languageadd.component.html',
  styleUrls: ['./languageadd.component.scss']
})
export class LanguageaddComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.id) {
      ...
      this.cmsservice.getSinglelanguage(newformData).subscribe(res => {
  ...
  userSubmit = async (formData) => {
    ...
    if (!this.id) {
        ...
        this.cmsservice.insertlanguage(newformData).subscribe((response) => {
          this.common.hideLoader()
        ...
        newformData.append('name_english', formData.name_english.trim());
        newformData.append('name_aramaic', formData.name_aramaic.trim());
        this.cmsservice.updatelanguage(newformData).subscribe((response) => {
          this.common.hideLoader()
          if (response['status'] == 1) {
           this.ngOnInit()
            this.toastrService.show('success', response['message'], { status: 'success' });
