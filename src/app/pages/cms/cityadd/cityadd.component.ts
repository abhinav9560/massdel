...
@Component({
  selector: 'ngx-cityadd',
  templateUrl: './cityadd.component.html',
  styleUrls: ['./cityadd.component.scss']
})
export class CityaddComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.id) {
      ...
      this.cmsservice.getSinglecity(newformData).subscribe(res => {
  ...
  userSubmit = async (formData) => {
    ...
    if (!this.id) {
        ...
        this.cmsservice.insertcity(newformData).subscribe((response) => {
          this.common.hideLoader()
        ...
        newformData.append('name_english', formData.name_english.trim());
        newformData.append('name_aramaic', formData.name_aramaic.trim());
        this.cmsservice.updatecity(newformData).subscribe((response) => {
          this.common.hideLoader()
          if (response['status'] == 1) {
           this.ngOnInit()
            this.toastrService.show('success', response['message'], { status: 'success' });
