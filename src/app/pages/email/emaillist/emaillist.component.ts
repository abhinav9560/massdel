...
export class EmaillistComponent implements OnInit {
  ...
  dateRange
  constructor(private common:CommonService,private emailservice: EmailService, private toastrService: NbToastrService) {
  ...
  getUsers = async (pageNo, searchItem, dateRange?) => {
    ...
    this.emailservice.get(UsersList).subscribe((res) => {
      this.data = res;
  ...
  InactiveUser = async (Id, indexIs) => {
    ...
    if (confirm('Are you sure you want to Change status?')) {
      ...
      await this.emailservice.inactive(newformData).subscribe((response) => {
  ...
  ActiveUser = async (Id, indexIs) => {
    if (confirm('Are you sure you want to Change status?')) {
      ...
      const newformData = new FormData();
      newformData.append('id', Id);
      await this.emailservice.active(newformData).subscribe((response) => {
        if (response['status'] == 1) {
          this.toastrService.show('success', response['message'], { status: 'success' });
          this.ngOnInit()
        } else {
  ...
  deleteUser = async (Id, Indexis) => {
    if (confirm('Are you sure you want to Delete?')) {
      ...
      await this.emailservice.delete(newformData).subscribe((response) => {
        if (response['status'] == 1) {
