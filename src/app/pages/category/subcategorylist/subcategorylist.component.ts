...
export class SubcategorylistComponent implements OnInit {
  ...
  getUsers = async (pageNo, searchItem, dateRange?) => {
    ...
    this.categoryService.getSubCategory(UsersList).subscribe((res) => {
      this.data = res;
  ...
  InactiveUser = async (Id, indexIs) => {
    ...
    if (confirm('Are you sure?')) {
      ...
      await this.categoryService.inactiveSubCategory(newformData).subscribe((response) => {
  ...
  ActiveUser = async (Id, indexIs) => {
    if (confirm('Are you sure?')) {
      ...
      const newformData = new FormData();
      newformData.append('id', Id);
      await this.categoryService.activeSubCategory(newformData).subscribe((response) => {
        if (response['status'] == 1) {
          this.toastrService.show('success', response['message'], { status: 'success' });
          this.ngOnInit()
        } else {
  ...
  deleteUser = async (Id, Indexis) => {
    if (confirm('Are you sure?')) {
      ...
      await this.categoryService.deleteSubCategory(newformData).subscribe((response) => {
        if (response['status'] == 1) {
  ...
  export() {
    ...
    this.categoryService.subcategoryexport(UsersList).subscribe((res) => {
      var data = res['data']
      if(data.length <= 0){
  ...
  statusChange(status) {
    if (confirm('Are you sure?')) {
      ...
      this.categoryService.subcategorymultiStatusChange(newformData).subscribe((res) => {
        console.log(res)
        if (res['status'] != -1) {
          this.checkAll = false
          this.ngOnInit()
  ...
  multiDelete() {
    if (confirm('Are you sure you?')) {
      const newformData = new FormData();
      newformData.append('idArray', JSON.stringify(this.checkedArray))
      this.categoryService.subcategorymultiDelete(newformData).subscribe((res) => {
        if (res['status'] != -1) {
          this.checkAll = false
          this.ngOnInit()
          this.toastrService.show('success', res['message'], { status: 'success' });
